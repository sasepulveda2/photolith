...
from PyQt5.QtWidgets import (
    ...
    QFileDialog, QSlider, QComboBox, QMessageBox
)
...
class LithographySimulator(QWidget):
    ...
    def load_pattern(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Seleccionar patrón", "", "Imágenes (*.png *.jpg *.bmp)")
        if file_path:
    ...
    def load_real_image(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Seleccionar imagen real", "", "Imágenes (*.png *.jpg *.bmp)")
        if file_path:
            image = cv2.imread(file_path, cv2.IMREAD_GRAYSCALE)
            self.real_image = image / 255.0
            QMessageBox.information(self, "Imagen cargada", "Imagen real cargada correctamente.")
    ...
    def compare_with_real(self):
        if self.real_image is None or not hasattr(self, 'last_intensity'):
            QMessageBox.warning(self, "error", "Carga una imagen real y genera una simulación primero.")
            return

        simulated_resized = cv2.resize(self.last_intensity, (self.real_image.shape[1], self.real_image.shape[0]))
